...
contains
  ...
  function compute_KPAR(Chl)

    !-----------------------------------------------------------------------
    !  compute attenuation factor of PAR
    !  based on Morel, Maritorena, JGR, Vol 106, No. C4, pp 7163--7180, 2001
    !-----------------------------------------------------------------------

    real(r8), intent(in) :: Chl            ! Chl-a concentration (mg Chl/m^3)
    real(r8)             :: compute_KPAR   ! attenuation factor (1/cm)

    !-----------------------------------------------------------------------
    !  local variables
    !-----------------------------------------------------------------------

    real (r8) :: Chl_cap   ! Chl, with lower bound enforced

    !-----------------------------------------------------------------------

    Chl_cap = max(Chl, 0.02_r8)
    if (Chl_cap < 0.13224_r8) then
      compute_KPAR = 0.000919_r8*(Chl_cap**0.3536_r8)
    else
      compute_KPAR = 0.001131_r8*(Chl_cap**0.4562_r8)
    end if

  end function compute_KPAR
  ...
  subroutine compute_PAR(domain, interior_tendency_forcings, interior_tendency_forcing_ind, &
    ...
    real (r8) :: layer_KPAR
    real (r8) :: layer_atten(domain%kmt)
    real (r8) :: cumulative_atten_top, log_cumulative_atten_top, cumulative_atten_bot
    ...
    associate(                                        &
      ...
      !-----------------------------------------------------------------------
      ! compute euphotic zone depth, zeu, where PAR is 1% of its surface value
      ! Compute by assuming a non-zero surface PAR value, and determine where
      ! attenuated PAR equals 1% of the surface value. The implementation
      ! simplifies if the surface value is 100, so that is done.
      !
      ! zeu equals the water column depth if water column is too shallow
      ! for attenuation to reduce below 1%.
      !-----------------------------------------------------------------------

      cumulative_atten_top = 100.0_r8
      log_cumulative_atten_top = log(100.0_r8)
      PAR%zeu = 0.0
      do k = 1, column_kmt
        layer_KPAR = compute_KPAR(totalChl_local(k))
        PAR%KPARdz(k) = layer_KPAR * delta_z(k)
        layer_atten(k) = exp(-PAR%KPARdz(k))
        cumulative_atten_bot = layer_atten(k) * cumulative_atten_top
        if (cumulative_atten_bot <= 1.0_r8) then
          PAR%zeu = PAR%zeu + log_cumulative_atten_top / layer_KPAR
          exit
        end if
        cumulative_atten_top = cumulative_atten_bot
        log_cumulative_atten_top = log_cumulative_atten_top - PAR%KPARdz(k)
        PAR%zeu = PAR%zeu + delta_z(k)
      end do

      !-----------------------------------------------------------------------
      ! avoid further computations if there is no light
      ...
      !-----------------------------------------------------------------------
      ! compute attenuation over column below euphotic zone
      !-----------------------------------------------------------------------
      ...
      do subcol_ind = 1, PAR_nsubcols
        if (PAR%interface(0,subcol_ind) > c0) then
          ...
          do k = 1, column_kmt
            PAR%interface(k,subcol_ind) = PAR%interface(k-1,subcol_ind) * layer_atten(k)
          ...
          do k = 1, column_kmt
            PAR%avg(k,subcol_ind) = PAR%interface(k-1,subcol_ind) * (c1 - layer_atten(k)) / PAR%KPARdz(k)
          end do
