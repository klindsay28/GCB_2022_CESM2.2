...
contains
  ...
  subroutine marbl_diagnostics_init(  &
    ...
    associate(                                         &
      ...
      lname = 'Euphotic Zone Depth, based on 1% PAR threshold'
      sname = 'zeu'
      units = 'cm'
      vgrid = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%zeu, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
  ...
  subroutine marbl_diagnostics_interior_tendency_compute( &
    ...
    call store_diagnostics_PAR(domain, &
         PAR%zeu, PAR%col_frac(:), PAR%avg(:,:), marbl_interior_tendency_diags)
  ...
  subroutine store_diagnostics_PAR(marbl_domain, zeu, PAR_col_frac, PAR_avg, &
       marbl_interior_diags)

    type(marbl_domain_type)      , intent(in)    :: marbl_domain
    real(r8)                     , intent(in)    :: zeu
