...
def path_replace(
    path_in, dir_replace=None, base_replace=None, date_trunc_len=None
):
    """perform string replacement on path components"""

    (dir, base) = os.path.split(path_in)
    if dir_replace is not None:
        for key, value in dir_replace.items():
            dir = dir.replace(key, value)
    if base_replace is not None:
        for key, value in base_replace.items():
            base = base.replace(key, value)
    if date_trunc_len is not None:
        words = base.split(".")
        words[-2] = "-".join(
            date[0:date_trunc_len] for date in words[-2].split("-")
        )
        base = ".".join(words)
